...
//
// `loom build` emits a self-contained SPA. Unlike the dev server's virtual
// modules (which lean on `/@fs/` URLs only Vite's dev middleware understands),
// the build writes three real files into a scratch entry dir so Rollup can
// follow every import: `index.html` → `entry.ts` → { globals, targets, shell }.
// Target imports use plain relative specifiers (computed against the scratch
// dir) so Rollup code-splits each `*.loom.tsx` into its own async chunk.
// ---------------------------------------------------------------------------
 ...
 * + stage + `#loom-root`) so the shell code is shared; `entrySpecifier` is the
 * module script (a relative `./entry.ts`).
 */
export function generateBuildIndexHtml(entrySpecifier: string): string {
	return `<!doctype html>
<html lang="en">
	<head>
		<meta charset="UTF-8" />
	...
	<body>
		...
		<script type="module" src=${JSON.stringify(entrySpecifier)}></script>
	</body>
</html>
`;
}
